...
class MainSectionCalibration {
    constructor (detConfig, webSock) {
        ...
        this.components = { xFactors: [], yFactors: [], zFactors: [], intFactors: [] };
    ...
    createPosCalibFields () {
        if (this.hasPos) {
            for(let i = 0; i<this.nChX; i++){
                this.components.xFactors.push(new NumberBox('calib_chX' + i, 'CH ' + i));
            }
            for(let j = 0; j<this.nChY; j++){
                this.components.yFactors.push(new NumberBox('calib_chY' + j, 'CH ' + j));
            }
        }
        if (this.hasInt) {
            for(let int = 0; int<this.nChInt; int++){
                this.components.intFactors.push(new NumberBox('calib_IntCh' + int, 'IntCH ' + int));
            }
        }
        if(this.devName == 'QPlus'){
            this.components.xFactors[(this.nChX-1)/2].handlerEvent('change', function () {
                let xval = !isNaN(validateNumInput(this.components.xFactors[(this.nChX-1)/2].getId(true))) ? validateNumInput(this.components.xFactors[(this.nChX-1)/2].getId(true)) : 1;
                this.components.xFactors[(this.nChX-1)/2].updateContent(xval);
                this.components.yFactors[(this.nChY-1)/2].update(xval);
            });
            this.components.yFactors[(this.nChY-1)/2].handlerEvent('change', function () {
                let yval = !isNaN(validateNumInput(this.components.yFactors[(this.nChY-1)/2].getId(true))) ? validateNumInput(this.components.yFactors[(this.nChY-1)/2].getId(true)) : 1;
